...
# 默认不写 __pycache__/，避免在 scripts/ 下生成 *.pyc 残留。
# `python3 -B` 的语义是"关闭字节码生成"，与本脚本默认行为一致；
# 想强制打开字节码生成时，反过来用 `python3`（不带 -B）。
import sys as _sys
...
DEFAULT_PLAN_OUTPUT = "合并清单.md"
DEFAULT_MERGED_FILENAME = "merged.xlsx"
MAX_SHEET_NAME_LEN = 31
# Excel sheet 名非法字符（\ / ? * : [ ]），写入前替换为 "_"
EXCEL_INVALID_SHEET_CHARS = re.compile(r"[\\/*?:[\]]")


# ---------- logger（默认 INFO；可由 --log-level 调整） ----------
...
def discover_files(directory, patterns, recursive=False, exclude_paths=None):
    ...
    if not directory.exists():
        logger.warning(f"路径不存在: {directory}")
        return []
...
def read_file(filepath, header=0, sheet_name=None, first_sheet_only=False):
    ...
    if suffix in (".xlsx", ".xls", ".xlsm"):
        try:
            ...
            logger.warning(f"无法读取 Excel 文件 {filepath}: {e}")
            return {}
    ...
    for section in sections[1:]:  # 跳过第一个（是标题部分）
        ...
        for line in lines:
            ...
            if stripped.startswith('- **文件列表**:'):
                in_file_list = True
                # A-2 P1-5：只支持每行一个路径（与生成器 generate_plan_markdown 对齐），
                # 不再用 ,/，/、 切分。冒号后若有内容，作为单条路径。
                files_str = stripped.split(':', 1)[1].strip()
                if files_str:
                    group['files'] = [files_str]
                else:
                    group['files'] = []
...
def merge_group(files, sheet_name, header=0, add_source=False, source_col="来源文件", all_columns=None):
    ...
    for filepath in files:
        ...
        if df is None or df.empty:
            logger.warning(f"跳过: {filepath.name} 中未找到 sheet '{sheet_name}' 或无数据")
            continue
...
def infer_output_path(directory, files_str, explicit_output):
    """
    推断输出路径（纯函数，不创建目录；目录创建由 main() / write_output 调用方负责）。
    - 如果用户显式指定了输出路径，直接使用
    ...
    return Path(DEFAULT_MERGED_FILENAME)
...
def _align_and_concat(dfs_with_name, add_source=False, source_col="来源文件", source_sheet=None, sources=None):
    """A-1 P1-1 / A-2 P1-3：把多份 DataFrame 按并集列对齐、追加来源列后垂直拼接。

    dfs_with_name: [(df, source_name), ...]
    - source_name 默认写进 source_col
    - 当 source_sheet 为 str 时，额外写一列 "来源Sheet"（单组只写一个值）
    - 当 sources 为 list[str | None] 时，按行写一列 "来源Sheet"，长度必须等于所有 df 行数之和
    返回: 单个 DataFrame。
    """
    if not dfs_with_name:
        return None
    all_columns = []
    seen = set()
    for df, _name in dfs_with_name:
        for col in df.columns:
            if col not in seen:
                seen.add(col)
                all_columns.append(col)
    aligned_dfs = []
    for df, name in dfs_with_name:
        aligned = pd.DataFrame()
        for col in all_columns:
            aligned[col] = df[col] if col in df.columns else pd.NA
        if add_source and source_col not in aligned.columns:
            aligned[source_col] = name
        if source_sheet is not None and "来源Sheet" not in aligned.columns:
            aligned["来源Sheet"] = source_sheet
        aligned_dfs.append(aligned)
    merged = pd.concat(aligned_dfs, ignore_index=True)
    if sources is not None:
        # A-2 P1-3：按行写一列 "来源Sheet"，与 concat 后的行数严格对齐
        if len(sources) != len(merged):
            raise ValueError(
                f"sources 长度 {len(sources)} 与合并后行数 {len(merged)} 不一致"
            )
        merged["来源Sheet"] = sources
    return merged
...
def merge_files_no_group(files_data, add_source, source_col):
    ...
    for sheet_name, df_list in sheet_data_map.items():
        merged_df = _align_and_concat(df_list, add_source=add_source, source_col=source_col)
        if merged_df is not None:
            merged[sheet_name] = merged_df
    return merged
...
def main():
    ...
    parser.add_argument(
        "--plan-output", default=DEFAULT_PLAN_OUTPUT,
        help=f"预览模式下合并清单的输出路径（默认: {DEFAULT_PLAN_OUTPUT}）"
    )
    parser.add_argument(
    ...
    if args.preview:
        ...
        # B 轮 P0-2：未显式指定 --plan-output 时，默认保存到当前工作目录的 ./合并清单.md，
        # 不再与自动推断的 output_path 同目录（避免用户去 子目录名-excel-merging/ 找）。
        if args.plan_output == DEFAULT_PLAN_OUTPUT:
            plan_path = Path(DEFAULT_PLAN_OUTPUT)
        else:
        ...
        if args.merge_sheets:
            ...
            for filepath in files:
                ...
                for gi, group in enumerate(groups, 1):
                    # A-1 P1-1：把「建 all_columns + 逐 sheet 按列对齐 + pd.concat」下放到
                    # _align_and_concat；--merge-sheets 模式与 merge_files_no_group 复用同一段。
                    dfs_in_group = [
                        (sheets[sheet_name], sheet_name)
                        for sheet_name, _cols in group
                    ]
                    if args.add_source:
                        # A-2 P1-3：把"每行带各自来源 sheet 名"通过 _align_and_concat 的
                        # sources 参数显式按行写出，移除主流程里的手工追加。
                        sources = []
                        for sheet_name, _cols in group:
                            df = sheets[sheet_name]
                            sources.extend([sheet_name] * len(df))
                    else:
                        sources = None
                    merged_df = _align_and_concat(
                        [(df, filepath.name) for df, _sn in dfs_in_group],
                        add_source=args.add_source,
                        source_col=args.source_col,
                        source_sheet=None,
                        sources=sources,
