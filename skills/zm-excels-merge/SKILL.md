...
- **智能字段分组**：默认模式下按 Jaccard 相似度自动分组，列名完全一致时直接合并，不一致时分组输出到独立 sheet
...
`/goal` 是会话层的任务跟踪能力（Codex CLI、Claude Code 等平台均提供），不是本 skill 的脚本参数。普通一次性合并可直接执行；当合并涉及多个文件、字段兼容性判断、清单调整或中断后续跑时，建议先开启 `/goal`。

- **目标记录**：写清输入范围、目标输出文件、是否递归、是否添加来源列、字段相似度阈值和验收标准。
- **阶段检查点**：按“发现文件 → 预览字段兼容性 → 调整合并清单 → 执行合并 → 抽样验证输出”推进。
- **恢复点**：若已生成 `合并清单.md`，续跑时优先读取该清单和当前 `/goal` 进度，不重新猜测分组意图。
...
2. 列名完全一致时直接合并；不一致时按 `--similarity-threshold`（默认 0.8）分组输出到独立 sheet
3. 每组缺失列自动补空值
...
3. 使用贪心算法分组：从第一个文件开始，找出所有相似度 >= 阈值的文件归入同一组
4. 对剩余文件重复上述过程
...
if input_path.is_file():
    ...
    files = list(input_path.glob('*.xlsx')) + list(input_path.glob('*.xls')) +             list(input_path.glob('*.xlsm')) + list(input_path.glob('*.csv'))
    files_by_suffix = {}
    for f in files:
    ...
    for suffix, suffix_files in files_by_suffix.items():
        ...
        for f in suffix_files:
            reader = pd.read_csv if f.suffix == '.csv' else                      lambda p: pd.read_excel(p, sheet_name=0, dtype=str, keep_default_na=False)
            df = reader(f)
            df['来源文件'] = f.name
            dfs.append(df)
        if dfs:
