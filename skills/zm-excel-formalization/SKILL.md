...
> 批量格式化、原地覆盖前确认、输出目录约束或跨会话继续等场景，建议在会话层用 `/goal` 跟踪进度（属会话层能力，非本 skill 入口）；单文件格式化通常直接执行。
...
# 真实生产请直接调用 scripts/format_excel.py::format_workbook，
# 它会原子替换临时文件并保留原文件属性。下面仅展示核心判定逻辑。
...
# 冲突时手动加编号；脚本默认走 generate_copy_path 自动递增，
# AI 推理路径务必显式处理，否则副本已存在会被静默覆盖
wb.save('data_副本.xlsx')  # 教学片段；生产请改用 format_workbook 并自行消歧
```
...
- 默认副本模式：使用 `generate_copy_path` 自动生成 `原名<copy_suffix>.<ext>`，冲突时递增 `<copy_suffix>1..N`；冲突编号上限 100 次，超出后报错并打印已尝试的候选路径

## 注意事项

- 仅处理 `.xlsx`、`.xlsm` 格式文件（大小写不敏感；openpyxl 不支持 Excel 97-2003 的 `.xls` 二进制格式；加密文件需先用 Excel 另存为非加密版本）
- ⚠️ openpyxl 写入 `.xlsm` 时会**剥离 VBA 宏**（含宏的业务模板如财务、报表自动化会丢失功能）；如需保留宏请改用 LibreOffice CLI / xlwings 等专用工具
- ⚠️ 本 skill 把含 CJK 字符的单元格统一应用"宋体"。`宋体` 在 Windows / macOS 默认预装；Linux 发行版需安装中文字体包（如 `fonts-wqy-microhei` / `fonts-noto-cjk`），否则 LibreOffice / WPS 会回退到默认字体。详见 README "## 字体依赖"
- 目录遍历仅处理当前层，不递归子目录；如需递归处理，请显式指定每个子目录
- 显式通配符 `*.xlsx` / `?.xlsx` 受支持；**递归通配符 `**/*.xlsx` 不被支持**（pathlib `match` 走 fnmatch 风格），如需递归请显式列出每个子目录
- 目录遍历默认跳过以 `.` 开头的隐藏文件
- 所有单元格均设置对齐样式（防止空行/空单元格被裁剪导致行数减少）；仅含值的单元格才设置字体
- 字号为 `None` 时回退到 11；字号为 `0`（合法值，表示继承默认）保持原值
- 批量处理时若单个文件失败，继续处理其余文件（最终若有失败文件，stderr 提示且退出码非零，便于 CI/CD 区分"全部成功 vs 部分失败"）
- `--dry-run` 模式下不进行运行时版本校验与资源预检（用户可能没有 openpyxl 也想预览路径；避免 dry-run 触发 N 次 xlsx 解析）
- 所有写入路径（`--in-place`、默认副本模式、`-o` 输出）均采用临时文件+原子替换：先写 `.{name}.{pid}.{idx}.tmp` 再 `os.replace`；主循环用 `try/finally` 兜底，确保 `KeyboardInterrupt` / `SystemExit` 也会清理临时文件，避免 Ctrl+C 取消时留下孤儿临时文件
- 列宽自动调整默认启用，按内容显示宽度计算：CJK 字符计为 2 单位，西文字符计为 1 单位；处理显式换行时取最长行的宽度；`wrap_text=True` 后的自动换行情况不参与估算（按显式换行优先）
- 损坏或加密 xlsx 会得到明确错误提示（不再暴露原始 openpyxl 异常）；如需保留宏请用 LibreOffice CLI / xlwings
